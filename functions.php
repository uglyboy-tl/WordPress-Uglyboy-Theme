...
// Custom Excerpts
function uglywp_index($length) // Create 20 Word Callback for Index page Excerpts, call using uglywp_excerpt('uglywp_index');
{
    return 20;
}

// Create 40 Word Callback for Custom Post Excerpts, call using uglywp_excerpt('uglywp_custom_post');
function uglywp_custom_post($length)
{
    return 40;
}

// Create the Custom Excerpts callback
function uglywp_excerpt($length_callback = '', $more_callback = '')
{
    global $post;
...
function ugly_blank_view_article($more)
{
    global $post;
...
function ugly_style_remove($tag)
{
...
add_action('init', 'register_ugly_menu'); // Add Uglyboy Menu
add_action('init', 'create_post_type_ugly'); // Add our Uglyboy Custom Post Type
add_action('widgets_init', 'my_remove_recent_comments_style'); // Remove inline Recent Comment Styles from wp_head()
add_action('init', 'uglywp_pagination'); // Add our ugly Pagination
...
add_filter('excerpt_more', 'ugly_blank_view_article'); // Add 'View Article' button instead of [...] for Excerpts
add_filter('show_admin_bar', 'remove_admin_bar'); // Remove Admin bar
add_filter('style_loader_tag', 'ugly_style_remove'); // Remove 'text/css' from enqueued stylesheet
...
// Shortcodes
add_shortcode('ugly_shortcode_demo', 'ugly_shortcode_demo'); // You can place [ugly_shortcode_demo] in Pages, Posts now.
add_shortcode('ugly_shortcode_demo_2', 'ugly_shortcode_demo_2'); // Place [ugly_shortcode_demo_2] in Pages, Posts now.

// Shortcodes above would be nested like this -
// [ugly_shortcode_demo] [ugly_shortcode_demo_2] Here's the page title! [/ugly_shortcode_demo_2] [/ugly_shortcode_demo]

/*------------------------------------*\
	Custom Post Types
\*------------------------------------*/

// Create 1 Custom Post type for a Demo, called Uglyboy
function create_post_type_ugly()
{
    register_taxonomy_for_object_type('category', 'uglyboy'); // Register Taxonomies for Category
    register_taxonomy_for_object_type('post_tag', 'uglyboy');
    register_post_type('uglyboy', // Register Custom Post Type
        array(
        'labels' => array(
...
function ugly_shortcode_demo($atts, $content = null)
{
    return '<div class="shortcode-demo">' . do_shortcode($content) . '</div>'; // do_shortcode allows for nested Shortcodes
}

// Shortcode Demo with simple <h2> tag
function ugly_shortcode_demo_2($atts, $content = null) // Demo Heading H2 shortcode, allows for nesting within above element. Fully expandable.
{
